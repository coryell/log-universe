...
// 0. Test getLocalized
assert.strictEqual(getLocalized({ "en-us": "Hello" }), "Hello", "getLocalized extracts en-us");
assert.strictEqual(getLocalized("Hello"), "", "getLocalized returns empty for strings");
...
assert.strictEqual(isMatch(redDwarf, "blue"), false, "blue does not match Red Dwarf");
...
assert.strictEqual(isMatch({ displayName: { "en-us": "Red" } }, "(Red)"), false, "(Red) query does NOT match Red target");
assert.strictEqual(isMatch(parenthesizedRed, "Red"), true, "Red query matches (Red) target");
...
assert.strictEqual(isMatch(redDwarf, "RED DWARF"), true, "RED DWARF matches Red Dwarf");
